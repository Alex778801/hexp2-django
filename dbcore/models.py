...
class Project(HierarchyOrderModelExt, SecurityModelExt):
    ...
    prefCostTypeGroup = models.ForeignKey('CostType', related_name='prefCostTypeGroup', verbose_name='Группа статей', null=True, blank=True,
                                          on_delete=models.SET_NULL)
    prefAgentGroup = models.ForeignKey('Agent', related_name='prefAgentGroup', verbose_name='Группа контрагентов', null=True, blank=True,
                                       on_delete=models.SET_NULL)
...
class FinOper(SecurityModelExt):
    ...
    costType = models.ForeignKey(CostType, related_name='costType', verbose_name='Статья', null=True, blank=False, on_delete=models.SET_NULL)
    agentFrom = models.ForeignKey(Agent, related_name='agentFrom', verbose_name='Агент Откуда', null=True, blank=True, on_delete=models.SET_NULL)
    agentTo = models.ForeignKey(Agent, related_name='agentTo', verbose_name='Агент Куда', null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def __str__(self):
        return str(self.project.pk or '') + ":" + str(self.project or '') + "|id:" +\
               str(self.pk) + "|" + str(self.costType or '') + "|" + str(self.notes or '') + " = " + str(self.amount or '')
...
class Budget(models.Model):
    ...
    def changeOrder(self, toOrder):
        ...
        if fromOrder > toOrder:
            recs = self.__class__.objects.filter(project=self.project, costType=self.costType, order__gte=toOrder, order__lt=fromOrder)
            for r in recs:
                r.order += 1
                r.save()
        else:
            recs = self.__class__.objects.filter(project=self.project, costType=self.costType, order__gt=fromOrder, order__lte=toOrder)
            for r in recs:
                r.order -= 1
                r.save()
        self.order = toOrder
