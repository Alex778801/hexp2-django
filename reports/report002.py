...
def report_2_moveOnCt(proj, beginDate, endDate, costType, agentFrom, agentTo):
    ...
    for y in year:
        yPart = {'ct': y['ct'], 'ctId': y['ctId'], 'isOut': y['isOut'], 'Ysum': y['summ'], 'Yqnty': y['qnty'],
                 'Ypr': y['pr']}
...
def report_2_detailOnCt(proj, beginDate, endDate, costType, agentFrom, agentTo):
    ...
    operList = list(
        ...
        .annotate(ct=F('costType__name'), ctId=F('costType__id'), isOut=F('costType__isOutcome'),
                  agF=F('agentFrom__name'), agT=F('agentTo__name'), summ=F('amount'), user=F('owner__username'),
                  userColor=F('owner__userattr__color'))
...
def report_2_detailOnAg(proj, beginDate, endDate, costType, agentFrom, agentTo, agentMode):
    ...
    operList = list(
        ...
        .annotate(ag=F(agentMode + '__name'), agId=F(agentMode + '__id'), ct=F('costType__name'),
                  user=F('owner__username'), summ=F('amount'), userColor=F('owner__userattr__color'))
...
# Отчет №2
def report002(projId, beginDate=-1, endDate=-1, costType=-1, agentFrom=-1, agentTo=-1):
    ...
    moveOnAg = [{'ag': 'О Т К У Д А  → → →', 'hdr': True}] + moveOnAgFrom + [
        {'ag': '→ → → К У Д А', 'hdr': True}] + moveOnAgTo
    # Детализация по статьям
